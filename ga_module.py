"""Leakage-aware genetic feature selection for EvoSelect."""
...
def make_individual(n_features: int, rng: random.Random, p_select: float = 0.25) -> list[int]:
    chromosome = [1 if rng.random() < p_select else 0 for _ in range(n_features)]
    ...
    *,
    ...
    try:
        cv_scores = cross_val_score(
            model, X.loc[:, mask], y, cv=cv, scoring="accuracy", n_jobs=1
        )
...
def _tournament(population, fitnesses, rng: random.Random, k: int):
...
def _mutate(chromosome, rate, rng: random.Random):
    child = [1 - bit if rng.random() < rate else bit for bit in chromosome]
    if not any(child):
...
def run_genetic_algorithm(
    ...
    if config.elite_size < 0 or config.elite_size >= config.pop_size:
        raise ValueError("elite_size must be >= 0 and smaller than pop_size.")
    ...
    for generation in range(config.generations):
        fitnesses = [
            _fitness(ind, X, y, config, cache=fitness_cache)
        ...
        elite_count = min(config.elite_size, config.pop_size - 1)
        ...
        while len(new_population) < config.pop_size:
            p1 = _tournament(
                population, fitnesses, rng, config.selection_pressure
            )
            p2 = _tournament(
                population, fitnesses, rng, config.selection_pressure)
            ...
            new_population.extend(
                [
                    _mutate(c1, config.mutation_rate, rng),
                    _mutate(c2, config.mutation_rate, rng),
                ]
            )
    ...
    return {
        ...
        "history": history,
        "best_chromosome": best_solution,
        "best_chromosome_str": "".join(map(str, best_solution)),
